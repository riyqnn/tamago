...
public entry fun pvp_pet(pet1: &mut Pet, pet2: &mut Pet, ctx: &mut TxContext) {
    assert!(!is_sleeping(pet1), E_PET_IS_ASLEEP);
    assert!(!is_sleeping(pet2), E_PET_IS_ASLEEP);

    let gb = get_game_balance();

    // Simple battle rule: siapa energi + happiness lebih tinggi = menang
    let score1 = (pet1.stats.energy as u64) + (pet1.stats.happiness as u64);
    let score2 = (pet2.stats.energy as u64) + (pet2.stats.happiness as u64);

    if (score1 >= score2) {
        // Pet1 menang
        pet1.game_data.experience = pet1.game_data.experience + gb.play_experience_gain;
        pet1.game_data.coins = pet1.game_data.coins + gb.work_coins_gain;

        // Pet2 kalah → energi berkurang
        if (pet2.stats.energy > gb.play_energy_loss) {
            pet2.stats.energy = pet2.stats.energy - gb.play_energy_loss;
        } else {
            pet2.stats.energy = 0;
        };

        emit_action(pet1, b"won_pvp");
        emit_action(pet2, b"lost_pvp");
    } else {
        // Pet2 menang
        pet2.game_data.experience = pet2.game_data.experience + gb.play_experience_gain;
        pet2.game_data.coins = pet2.game_data.coins + gb.work_coins_gain;

        // Pet1 kalah → energi berkurang
        if (pet1.stats.energy > gb.play_energy_loss) {
            pet1.stats.energy = pet1.stats.energy - gb.play_energy_loss;
        } else {
            pet1.stats.energy = 0;
        };

        emit_action(pet1, b"lost_pvp");
        emit_action(pet2, b"won_pvp");
    };
}
